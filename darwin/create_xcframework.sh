...
DEPENDENCY_LIBS=(
  ...
  "libggml-cpu.dylib" "libggml-blas.dylib" "libmtmd.dylib" "libllama.dylib" 
)
...
if [[ -z "$IDENTITY" ]]; then
  echo "🔓  No signing identity supplied – will use adhoc signing"
else
...
codesign_bin() {
  ...
  if [[ -z "$IDENTITY" ]]; then
    # For adhoc/simulator - clean sign without preserve-metadata
    if /usr/bin/codesign --force --sign - "$bin" &>/dev/null; then
...
build_slice() {
  ...
  # Fix libllama dependency in main executable
  install_name_tool -change "@rpath/libllama.dylib" \
                    "@loader_path/Frameworks/libllama.dylib" \
...
strip_signature "$IOS_SIM_FW/$EXECUTABLE_NAME"
codesign_bin "$IOS_SIM_FW/$EXECUTABLE_NAME"
for DEP in "${DEPENDENCY_LIBS[@]}"; do
  [[ -f "$IOS_SIM_FW/Frameworks/$DEP" ]] || continue
  strip_signature "$IOS_SIM_FW/Frameworks/$DEP"
  codesign_bin "$IOS_SIM_FW/Frameworks/$DEP"
done
echo
...
printf "┌%-38s┬%-12s┐\n" " Binary " "Team ID"
for BIN in \
  "$OUTPUT_XCFW"/ios-arm64_x86_64-simulator/"$FRAMEWORK_NAME".framework/$EXECUTABLE_NAME \
  "$OUTPUT_XCFW"/ios-arm64_x86_64-simulator/"$FRAMEWORK_NAME".framework/Frameworks/*.dylib
do
...
[[ -z "$IDENTITY" ]] \
  && echo "🔓  Framework signed with adhoc for simulator" \
